...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (!req.url.includes('https://') && req.url.startsWith('/https:/a')) {
      let commonOne = req.url;
      req.url = commonOne.replace('https:/', 'https://')
    }
    ...
    if (!location) {
      ...
      if (/^\/https?:\/[^/]/i.test(req.url)) {
        ...
        res.end(`The URL is invalid: two slashes are needed after the http(s):., ${req.url} ${req.url.slice(1)}`);
        return;
      }
